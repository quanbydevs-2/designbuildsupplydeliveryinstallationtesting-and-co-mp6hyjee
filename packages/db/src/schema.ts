...
export const livestocks = createTable("livestocks", t => ({
  ...
  vaccinationStatus: t.text("vaccination_status").notNull(),
  status: t.text("status").notNull().default("active"),
  authorId: t.text("author_id").references(() => users.id, { onDelete: "set null" }),
  createdAt: t.timestamp("created_at").notNull().defaultNow(),
  updatedAt: t.timestamp("updated_at").notNull().defaultNow(),
...
export const landparcels = createTable("landparcels", t => ({
  ...
  tenureType: t.text("tenure_type").notNull(),
  status: t.text("status").notNull().default("active"),
  authorId: t.text("author_id").references(() => users.id, { onDelete: "set null" }),
  createdAt: t.timestamp("created_at").notNull().defaultNow(),
  updatedAt: t.timestamp("updated_at").notNull().defaultNow(),
...
export const relations = defineRelations({ users, sessions, accounts, farmers, crops, livestocks, programs, landparcels }, r => ({
  users: {
...
export const schema = Object.assign({ users, sessions, accounts, verifications, farmers, crops, livestocks, programs, landparcels }, relations)
